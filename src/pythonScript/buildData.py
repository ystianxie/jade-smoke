...
def to_excel(items,tag):
    ...
    with pandas.ExcelWriter(file_name, engine='xlsxwriter', engine_kwargs={'options': {"strings_to_urls": False}}) as writer:
...
def field_add():
    ...
    repeat_operation = config_data['repeat_operation']
    items = []
    slave_items = {}
    """
    {
    ...
    for master_row in master_excel:
        # 去除不导出的字段
        item = {key: master_row.get(key) for key in config_data['export_key'] if key}
        # 确保匹配键存在
...
def joint_deduplication():
    ...
    cache = set()
    for master_row in master_excel:
        only_key = "###".join([master_row.get(k,"") for k in config_data['only_keys']])
        if config_data['hold'] == "First":
            ...
            items_[only_key] = {key: master_row.get(key) for key in export_key if key}
    if config_data['hold'] != "First":
